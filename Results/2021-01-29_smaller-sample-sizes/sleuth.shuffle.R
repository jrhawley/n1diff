...
select_samples <- function(metadata, total) {
    ...
    usf_mu_ids <- metadata[condition == "mu", sample(sample, 1)]
    usf_ctrl_ids <- metadata[condition == "ctrl", sample(sample, total - 1)]
...
jse_shrinkage <- function(obj, s_targets = NULL, which_sample, which_beta, which_fit = "full", shrinkage_coef = NULL, which_var = "obs_counts") {
    ...
    jse_var <- (
        ...
        + ( k^2 + 2 * shrinkage_numerator / (denom ^ 2) - 1 ) * (b1 %*% t(b1))
    )
        ...
        for (k in c("balanced", "unbalanced")) {
            ...
            if (k == "unbalanced") {
                ...
                so_jse <- jse_shrinkage(
                    ...
                    which_sample = sampled_metadata[[j]][[k]][, .N, by = "condition"][N == 1, sample],
